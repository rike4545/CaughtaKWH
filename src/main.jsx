      ...
      <div className="content" ref={detailRef}>
        ...
        <Card>
          ...
          <div className="leadInsights">
            <div className="leadIns cheap">
              <div className="leadIcon" aria-hidden="true"><Clock3 size={20}/></div>
              <div className="leadBody"><span className="leadK">Cheapest charging window</span><strong className="leadV">{prediction ? bestWindowLabel : '—'}</strong><span className="leadS">{prediction ? 'lowest expected $/kWh across the day' : 'needs more price history'}</span></div>
            </div>
            {(() => {
              const q = prediction?.availability?.hasData ? prediction.availability.quietestSlot : null;
              return <div className="leadIns open">
                <div className="leadIcon" aria-hidden="true"><BatteryCharging size={20}/></div>
                <div className="leadBody"><span className="leadK">Best shot at an open stall</span><strong className="leadV">{q ? q.label : '—'}</strong><span className="leadS">{q ? `${q.expectedAvailableStalls != null ? `~${q.expectedAvailableStalls} open · ` : ''}${Math.round(q.expectedOccupancyPct * 100)}% full typically` : 'needs stall-count data'}</span></div>
              </div>;
            })()}
          </div>
          <div className="priceStrip"><div><span>Last observed</span><strong>{prediction?.latestObservedAt ? freshnessLabel(prediction.latestObservedAt) : 'Never'}</strong><small>{prediction?.latestObservedAt ? `${shortDate(prediction.latestObservedAt)} · ${publicCheckResult}` : 'No public price yet'}</small></div><div><span>Non-Tesla premium</span><strong>{priceSpreadCents != null ? `${signedCents(priceSpreadCents)}/kWh` : '—'}</strong><small>{priceSpreadCents != null ? 'over the Tesla / member rate' : 'need both rates'}</small></div><div><span>How much to trust it</span><strong>{prediction?.confidenceLabel || 'Low'}</strong><small>{prediction?.confidenceScore != null ? `${prediction.confidenceScore}/100 · ${prediction.sampleCount} samples` : 'Needs more samples'}{prediction?.neuralModel?.status ? ` · NN ${prediction.neuralModel.status}${prediction.neuralModel.holdoutMae != null ? `, ${(prediction.neuralModel.holdoutMae * 100).toFixed(1)}¢ MAE` : ''}${prediction.neuralModel.historicalCapturesFlagged ? ` · ${prediction.neuralModel.historicalCapturesFlagged} capture flagged` : ''}` : ''}</small></div><div><span>Stalls and speed</span><strong>{selected?.stalls || '—'} stalls</strong><small>{selected?.maxKw ? `Up to ${selected.maxKw} kW` : selected?.capacityConfidence || 'capacity unknown'}</small></div></div>
